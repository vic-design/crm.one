...
const openDialog = (role: RoleEditPayload) => {
...
const submit = () => {
    ...
    // Вызываем .url у сгенерированного роута с передачей аргумента ID
    form.put(roles.update(currentRoleId.value).url, {
        onSuccess: () => {
            isOpen.value = false;
        },
    });
        ...
        <DialogContent class="max-w-2xl max-h-[85vh] overflow-y-auto bg-zinc-950 border-zinc-800 text-zinc-100">
            ...
            <form @submit.prevent="submit" class="space-y-6 py-4">
                <div class="space-y-2">
                    ...
                    <input v-model="form.name" type="text" class="w-full px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-md text-zinc-100 focus:outline-none focus:border-zinc-700" />
                    <p v-if="form.errors.name" class="text-sm text-red-500">{{ form.errors.name }}</p>
                </div>

                <div class="space-y-4">
                    <label class="text-sm font-medium text-zinc-300 block">Разрешения роли</label>
